...
## 🌟 Χαρακτηριστικά (Features v1.6.1)

* 📏 **Strict Step Versioning Rule (+0.0.1) (v1.6.1)**: Θέσπιση αυστηρού κανόνα για σταδιακή αύξηση της έκδοσης αποκλειστικά κατά `0.0.1` σε κάθε αλλαγή κώδικα (π.χ. `1.6.0` -> `1.6.1`), απαγορεύοντας τα απότομα άλματα εκδόσεων.
* 🛠️ **Αντιμετώπιση Προβλημάτων Σύνδεσης & Link Δημιουργού (v1.6.0)**:
  * Προσθήκη αναλυτικής ενότητας **Αντιμετώπισης Προβλημάτων (Troubleshooting)** στις Οδηγίες (έλεγχος Bluetooth, κλείσιμο εφαρμογής Xiaomi Home/Mi Home στο κινητό λόγω Single Connection Rule, έλεγχος μπαταρίας/απόστασης).
  * Προσθήκη ενεργού συνδέσμου [SpyAlekos](https://alekos.program.gr) προς την προσωπική ιστοσελίδα.
* 🎨 **White & Bold Legibility Styling (v1.5.0)**: Τα γράμματα και τα εικονίδια στους δείκτες της μεγιστοποιημένης προβολής (Δείκτης Άνεσης & Υγεία Φυτού) έγιναν **λευκά (`#FFFFFF`) και έντονα (`Bold`)** πάνω από τα έγχρωμα φόντα (πράσινο, πορτοκαλί, κόκκινο, μπλε), εξασφαλίζοντας τέλεια αναγνωσιμότητα.
* ❓ **Κουμπί Βοήθειας & Οδηγιών (v1.4.0)**: Προσθήκη διαδραστικού κουμπιού βοηθείας στην κεφαλίδα της εφαρμογής, το οποίο ανοίγει παράθυρο διαλόγου με αναλυτικές οδηγίες χρήσης, επεξήγηση των μεθόδων BLE, λειτουργία μεγιστοποίησης και ρυθμίσεις.
* 🔲 **Flet Maximize & Focus Mode (v1.3.0)**: Δυνατότητα μεγιστοποίησης (Fullscreen Focus) είτε του Θερμομέτρου είτε του αισθητήρα Mi Flora με ένα κλικ. Πλήρης εξάλειψη του γκρι πλαισίου (Gray Box Layout Fix) με αφαίρεση `expand=True` από όλα τα στοιχεία του scrollable view. Στη μεγιστοποιημένη προβολή παρέχονται:
...
## 👤 Δημιουργός
Ανάπτυξη από [SpyAlekos](https://alekos.program.gr)
